/* ============================================================
   ...
   Supports dual-agent (Planner + Coder) architecture.
   ============================================================ */
...
/** A single task prompt generated by the Planner for the Coder to execute. */
export interface TaskPrompt {
  id: string;
  targetFile: string;
  purpose: string;
  inputFiles: { file: string; reason: string; extract: string }[];
  connections: string;
  contentRequirements: string;
  designRequirements: string;
  technicalConstraints: string;
  writtenSoFar: string; // summary of files completed before this task
}
...
interface AIState {
  ...
  activePersona: "Coder" | "Architect" | "Debugger" | "Reviewer" | "Documenter" | "Planner";
  isStreaming: boolean;
  ...
  agentStatus: "idle" | "thinking" | "reading" | "executing" | "generating" | "planning";
  agentPhase: "idle" | "planning" | "coding"; // which brain is active
  ...
  // Dual-Agent (Planner + Coder) State
  agentArchitecture: "planner-coder" | "monolithic";
  plannerTaskQueue: TaskPrompt[];
  currentTaskIndex: number;
  plannerModel: string; // can differ from activeModel
  ...
  setActivePersona: (persona: "Coder" | "Architect" | "Debugger" | "Reviewer" | "Documenter" | "Planner") => void;
  ...
  setAgentStatus: (status: "idle" | "thinking" | "reading" | "executing" | "generating" | "planning") => void;
  setAgentPhase: (phase: "idle" | "planning" | "coding") => void;
  ...
  // Dual-Agent Actions
  setAgentArchitecture: (arch: "planner-coder" | "monolithic") => void;
  setPlannerTaskQueue: (tasks: TaskPrompt[]) => void;
...
export const useAIStore = create<AIState>((set) => ({
  ...
  // Dual-Agent defaults
  agentArchitecture: "planner-coder",
  plannerTaskQueue: [],
  currentTaskIndex: 0,
  plannerModel: "", // empty = use activeModel
  ...
  // Dual-Agent Actions
  setAgentArchitecture: (agentArchitecture) => set({ agentArchitecture }),
  setPlannerTaskQueue: (plannerTaskQueue) => set({ plannerTaskQueue }),
